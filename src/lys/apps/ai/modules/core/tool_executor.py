...
Handles tool execution with guardrails, argument conversion, and result serialization.
"""

import inspect
import json
import logging
from typing import Dict, Any, Optional, get_origin, get_args, Union
...
class ToolExecutor:
    """
    Executes tools registered in the app registry.
    ...
    async def execute(
        ...
        if tool_name == "confirm_action":
            return await cls._handle_confirm_action(tool_args, session, info)

        # Handle navigate as a frontend passthrough tool
        if tool_name == "navigate":
            return cls._handle_navigate(tool_args, info)
        ...
        guardrail_result = await cls._process_guardrail(
        ...
        # Execute the tool
        return await cls._execute_internal(
    ...
    async def _handle_confirm_action(
        ...
        if result.get("status") == "execute":
            executed_result = await cls._execute_internal(
                result["tool_name"],
                result["tool_data"],
                result["tool_args"],
                session,
    ...
    @classmethod
    def _handle_navigate(cls, tool_args: Dict[str, Any], info: Any) -> Dict[str, Any]:
