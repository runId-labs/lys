"""
Pytest configuration for organization integration tests.

Provides a session-scoped AppManager with organization app loaded,
including all required base data for testing client and user services.
"""
...
async def organization_app_manager():
    """Create AppManager with organization app loaded."""
    ...
    async with app_manager.database.get_session() as session:
        ...
        # Roles (needed for organization tests)
        role_service = app_manager.get_service("role")
        role_webservice_entity = app_manager.get_entity("role_webservice")

        # Normal role (can be assigned to client users)
        normal_role = await role_service.create(
            session=session, id="NORMAL_ROLE", enabled=True, supervisor_only=False
        )
        rw = role_webservice_entity(role_id=normal_role.id, webservice_id="some_webservice")
        session.add(rw)

        # Supervisor-only role (cannot be assigned to client users)
        await role_service.create(
            session=session, id="SUPERVISOR_ROLE", enabled=True, supervisor_only=True
        )

        # Additional role for update tests
        await role_service.create(
