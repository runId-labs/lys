...
| H8 | [Service-to-Service Token Without Instance Identity](#h8-service-to-service-token-without-instance-identity--done) | HIGH | Sprint+1 |
| H9 | [No Audit Logging on Private Data Access](#h9-no-audit-logging-on-private-data-access--done) | HIGH | Sprint+1 |
| M1 | [Missing HTTP Security Headers](#m1-missing-http-security-headers--done) | MEDIUM | Before prod |
| M2 | [No Global API Rate Limiting](#m2-no-global-api-rate-limiting--done) | MEDIUM | Sprint+1 |
| M3 | [Debug Mode Active in DEMO Environment](#m3-debug-mode-active-in-demo-environment--done) | MEDIUM | Before prod |
| M4 | [Insecure Cookie Defaults](#m4-insecure-cookie-defaults--done) | MEDIUM | Before prod |
| M5 | [Mass Assignment in EntityService.create()](#m5-mass-assignment-in-entityservicecreate--done) | MEDIUM | Sprint+2 |
| M6 | [Unbounded Search Parameters](#m6-unbounded-search-parameters--done) | MEDIUM | Sprint+1 |
...
| M8 | [Super User Bypass Without Audit Trail](#m8-super-user-bypass-without-audit-trail--done) | MEDIUM | Sprint+1 |
| M9 | [No SSL/TLS Enforcement for Database Connections](#m9-no-ssltls-enforcement-for-database-connections--done) | MEDIUM | Sprint+1 |
...
## H8. Service-to-Service Token Without Instance Identity — DONE

**File:** `src/lys/core/utils/auth.py`

### The Problem

Service tokens contained `service_name` and `type` but no instance identifier. If the
...
Added an `instance_id` parameter to `ServiceAuthUtils.__init__()`:

- Auto-generated as a truncated UUID4 (8 chars) if not provided
- Can be explicitly set (e.g., from `HOSTNAME` or Kubernetes pod name)
- Included in every generated service token as the `instance_id` claim
- Stable across all tokens generated by the same instance
- No validation required on decode — the claim is for traceability, not authentication

This enables tracing which specific instance made each call, useful for incident response.

---

## H9. No Audit Logging on Private Data Access — DONE

**Files:**
- `src/lys/core/entities.py` — `_sensitive` attribute on `Entity`
- `src/lys/core/utils/access.py` — audit log for individual access (getter, edition, delete, creation)
- `src/lys/core/permissions.py` — audit log for list access (connections) + `webservice_name` populated in context
- `src/lys/core/graphql/fields.py` — audit log for `lys_field` (services backed by a sensitive entity)
- `src/lys/apps/user_auth/modules/user/entities.py` — `User` and `UserPrivateData` marked as sensitive

### The Problem

When `user_accessing_filters()` is applied and a user tries to access another user's private
data, the query simply returns no results. There was no log of the attempt. An attacker could
iterate through user IDs without detection.

### Fix Applied

Added a `_sensitive` class attribute on `Entity` (default `False`) and audit logging at
three points covering all GraphQL access patterns:

- **`check_access_to_object()`** — logs after permission check succeeds for individual access
  (getter, edition, delete, creation via `lys_getter`, `lys_edition`, `lys_delete`, `lys_creation`)
- **`add_access_constraints()`** — logs when list access is granted (connections via `lys_connection_field`)
- **`lys_field` resolver** — logs when a `lys_field` resolver is backed by a service whose
  `entity_class._sensitive` is `True` (inherits sensitivity from the entity)

Each audit log includes: entity name, entity ID (when available), user ID (or `None` for
anonymous access), webservice name, and access type.

Logging even when `connected_user` is `None` allows detecting permission configuration
errors during development (anonymous access to sensitive entities).

Any new entity with private data can opt in by setting `_sensitive = True`.

---

## M1. Missing HTTP Security Headers — DONE

**File:** `src/lys/core/middlewares.py`
...
Added `SecurityHeadersMiddleware` in `src/lys/core/middlewares.py` that sets the following
...
- `Strict-Transport-Security: max-age=31536000; includeSubDomains` — HSTS (only on HTTPS)

The middleware follows the existing pattern (`MiddlewareInterface` + `BaseHTTPMiddleware`)
and is registered via `settings.middlewares` with the path
`"lys.core.middlewares.SecurityHeadersMiddleware"`.

---

## M2. No Global API Rate Limiting — DONE
...
Added `RateLimitMiddleware` in `src/lys/core/middlewares.py` with dual backend:

- **Redis backend** (distributed): Uses `app_manager.pubsub._async_redis` when available.
  Rate limiting via `INCR` + `EXPIRE` (fixed window per IP). Works across multiple instances.
- **In-memory fallback**: When Redis is not configured, uses a local dict of timestamps.
  Sufficient for single-instance deployments and development.
- **Graceful degradation**: If Redis raises an error, the request is allowed through
  (same pattern as `PubSubManager.set_if_not_exists`).
...
Returns HTTP 429 with `Retry-After: 60` header when limit is exceeded.

Constant `RATE_LIMIT_PLUGIN_KEY` defined in `src/lys/core/consts/plugins.py`.

---

## M3. Debug Mode Active in DEMO Environment — DONE

**File:** `src/lys/core/configs.py`
...
Changed the `debug` property to only return `True` for `DEV`:

```python
return self.env == EnvironmentEnum.DEV
```

DEMO, PREPROD, and PROD all have `debug=False`.

---

## M4. Insecure Cookie Defaults — DONE

**File:** `src/lys/apps/user_auth/modules/auth/services.py`

### The Problem

Cookie security flags were loaded from configuration with no defaults. If the auth plugin
was not fully configured, cookies were sent without `secure`, `httponly`, or `samesite` flags.

### Fix Applied

Added secure default values to `set_cookie()`:

- `cookie_secure` → default `True` (cookies only sent over HTTPS)
- `cookie_http_only` → default `True` (JavaScript cannot read cookies)
- `cookie_same_site` → default `"Lax"` (prevents CSRF on POST, allows GET navigation)

Explicit configuration still overrides defaults.

---

## M5. Mass Assignment in EntityService.create() — DONE

**File:** `src/lys/core/services.py`

### The Problem

`create()` and `update()` passed all kwargs directly to the entity constructor/update
statement. This allowed setting relationships (e.g., `client=<object>` instead of
`client_id="uuid"`) or non-column attributes.

### Fix Applied

Added `_filter_allowed_fields()` which introspects `entity_class.__table__.columns`
to determine valid fields. Applied to both `create()` and `update()`:

- Only actual database columns are accepted (not relationships, hybrid properties, etc.)
- Unexpected fields are logged with a warning and silently dropped
- Zero configuration required — introspection is automatic for all EntityService subclasses
- No existing code affected (audit confirmed all 32 create + 1 update calls use columns only)

---

## M6. Unbounded Search Parameters — DONE

**Files:**
- `src/lys/core/utils/validators.py` — `validate_search_input()` function
- All 8 search call sites across webservices and services

### The Problem

Search parameters were used in `ILIKE` queries without length validation. A very long search
string with wildcards generates an expensive database query that can cause DoS.

### Fix Applied

Added `validate_search_input()` in `src/lys/core/utils/validators.py`:

- Rejects search strings longer than 200 characters (`SEARCH_TOO_LONG_ERROR`)
- Strips SQL wildcards (`%` and `_`) to prevent expensive LIKE scans
- Returns `None` passthrough for `None` input

Applied to all 8 ILIKE search points:
- `user_auth/modules/user/webservices.py` (2 occurrences)
- `organization/modules/user/webservices.py` (2 occurrences)
- `organization/modules/client/webservices.py` (1)
- `user_role/modules/user/webservices.py` (1)
- `licensing/modules/user/webservices.py` (1)
- `base/modules/log/webservices.py` (1)
- `user_auth/modules/user/services.py` (1 — email_search)
...
## M8. Super User Bypass Without Audit Trail — DONE
...
## M9. No SSL/TLS Enforcement for Database Connections — DONE

**Files:**
- `src/lys/core/configs.py` — `DatabaseSettings.ssl_mode` attribute
- `src/lys/core/managers/database.py` — SSL injection in `_get_engine_kwargs()`

### The Problem

Database connections were established without SSL by default. In a cloud environment,
traffic between the application and database may traverse untrusted networks.

### Fix Applied

Added `ssl_mode` attribute to `DatabaseSettings` (default: `"require"`) for secure-by-default
database connections. In `_get_engine_kwargs()`, when the database type is PostgreSQL:

- **Async mode (asyncpg)**: injects `ssl` key into `connect_args`
- **Sync mode (psycopg2)**: injects `sslmode` key into `connect_args`
- Uses `setdefault()` so explicit `connect_args` SSL config is never overridden
- Logs a warning when `ssl_mode` is `None` (disabled)

Configuration:
- Default: `ssl_mode="require"` (SSL required, server certificate not verified)
- Production: `ssl_mode="verify-full"` (full certificate verification)
- Development: `ssl_mode=None` (disable for local PostgreSQL without SSL)
- SQLite/MySQL: `ssl_mode` is ignored for non-PostgreSQL databases
...
| H8 | Instance identity in service tokens | [x] |
| H9 | Audit logging on private data access | [x] |
 | M1 | Security headers middleware | [x] |
| M2 | Global API rate limiting | [x] |
| M3 | Debug mode only in DEV | [x] |
| M4 | Secure cookie defaults | [x] |
| M5 | Mass assignment protection | [x] |
| M6 | Bounded search parameters | [x] |
| M7 | Generic error messages in Mollie endpoint | [x] |
| M8 | Super user audit trail | [x] |
| M9 | Database SSL/TLS | [x] |